...
forecast_api_key = ENV["wkey"]

# Latitude, Longitude for location
forecast_location_lat = "48.9833878"
forecast_location_long = "1.9722383"
...
SCHEDULER.every '5m', :first_in => 0 do |job|
  ...
  puts "/forecast/#{forecast_api_key}/#{forecast_location_lat},#{forecast_location_long}?units=#{forecast_units}"
  ...
  # rala = { current_temp: "#{forecast_current_temp}&deg;", current_icon: "#{forecast_current_icon}", current_desc: "#{forecast_current_desc}", next_icon: "#{forecast_next_icon}", next_desc: "#{forecast_next_desc}", later_icon: "#{forecast_later_icon}", later_desc: "#{forecast_later_desc}"}
  # puts rala.inspect
